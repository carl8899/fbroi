...
angular.module('carl8899.controllers')
	.controller('statisticsAdSetController', ['$rootScope', '$scope', '$q', 'Auth', 'Utils', 'AdSet', function($rootScope, $scope, $q, Auth, Utils, AdSet) {
		...
		$scope.render = function(which) {
			if(which == '' || which == 'GRAPH') {
				$scope.graphdata = $scope.renderGraph($scope.adsets, $scope.filters);	
			}
		...
		$scope.onApplySelected= function() {
			...
			_.each($scope.checkedItems, function(item) {
				$promises.push(updateItem(new AdSet(item)));
			});
